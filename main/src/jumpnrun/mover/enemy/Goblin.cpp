...
void Goblin::initialize(int refx, int refy
	, gmtr::Vector2D p, OWNERHANDLE owner, bool turned)
{
	...
	writeName("character_enemy_goblin", name);
}

void Goblin::run(jnr::JumpnRunData& data)
{
	Enemy::run(data);
...
void Goblin::actz(phsc::Physics& phys, jnr::JumpnRunData& data)
{
	if (isOnSomething)
	{
		//phys.v.y = -speed_jump;
		...
		animation.setSpell(spl::eSpell::spl_bomb, 40);
...
void Goblin::think(jnr::JumpnRunData& data)
{
	if (ai_timer > 0)
	{
		ai_timer--;
