	...
	, gmtr::Vector2D p, OWNERHANDLE owner, bool turned)
{
	...
	writeName("character_enemy_pylonman", name);
}

void Pylonman::drive(phsc::Physics& phys, jnr::JumpnRunData& data)
{
...
void Pylonman::run(jnr::JumpnRunData& data)
{
	timer_guard.advance();
...
int Pylonman::damage(const Damage& d)
{
...
void Pylonman::think(jnr::JumpnRunData& data)
{
	EnemySearcher es;
	es.initialize(this);
	auto v = es.search(144);
	auto o = this->Owner();
...
void Pylonman::render(gfw::BaseMedia& media, jnr::JumpnRunData& data)
{
