...
RegisterMoverClass(Bicycle);

void Bicycle::initialize(int refx, int refy
	, gmtr::Vector2D p, OWNERHANDLE owner, bool turned)
{
	...
	writeName("character_enemy_bicycle", name);
}

void Bicycle::run(jnr::JumpnRunData& data)
{
	Enemy::run(data);
...
void Bicycle::actz(phsc::Physics& phys, jnr::JumpnRunData& data)
{
	/*if (isOnSomething)
	{
		//phys.v.y = -speed_jump;
...
void Bicycle::think(jnr::JumpnRunData& data)
{
	if (ai_timer > 0)
	{
		ai_timer--;
...
void Bicycle::render(gfw::BaseMedia& media, jnr::JumpnRunData& data)
{
	BaseGraphics* graphics = media.getGraphics();
	int offset_x = ((age / 8) % 2) * (phys.v.x != 0.0);
