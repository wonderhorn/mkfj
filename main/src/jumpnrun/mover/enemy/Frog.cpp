...
void Frog::initialize(int refx, int refy
	, gmtr::Vector2D p, OWNERHANDLE owner, bool turned)
{
	...
	writeName("character_enemy_frog", name);
}

void Frog::drive(phsc::Physics& phys, jnr::JumpnRunData& data)
{
...
void Frog::run(jnr::JumpnRunData& data)
{
	Enemy::run(data);
...
void Frog::think(jnr::JumpnRunData& data)
{
	if (ai_timer)
	{
		ai_timer--;
...
void Frog::render(gfw::BaseMedia& media, jnr::JumpnRunData& data)
{
