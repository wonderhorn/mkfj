...
RegisterMoverClass(Bat);

void Bat::initialize(int refx, int refy
	, gmtr::Vector2D p, OWNERHANDLE owner, bool turned)
{
	...
	direction = 3;
	this->turned = turned;
	writeName("character_enemy_bat", name);
}

void Bat::drive(phsc::Physics& phys, jnr::JumpnRunData& data)
{
...
void Bat::run(jnr::JumpnRunData& data)
{
	Enemy::run(data);
...
void Bat::think(jnr::JumpnRunData& data)
{
	if (ai_timer)
	{
		ai_timer--;
...
void Bat::render(gfw::BaseMedia& media, jnr::JumpnRunData& data)
{
	BaseGraphics* graphics = media.getGraphics();
	int offset_x = age % 6 < 3 + 1;
	if (v_reaction.y > 0)
		offset_x = 0;
