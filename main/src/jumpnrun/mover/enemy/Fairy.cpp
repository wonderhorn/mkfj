...
RegisterMoverClass(Fairy);

void Fairy::initialize(int refx, int refy
...
void Fairy::drive(phsc::Physics& phys, jnr::JumpnRunData& data)
{
...
void Fairy::run(jnr::JumpnRunData& data)
{
	Enemy::run(data);
...
void Fairy::actz(phsc::Physics& phys, jnr::JumpnRunData& data)
{
	//if (isOnSomething)
	{
		//phys.v.y = -speed_jump;
		...
		animation.setSpell(spl::eSpell::spl_bubblebarrier, 30);
...
void Fairy::think(jnr::JumpnRunData& data)
{
	if (ai_timer)
	{
		ai_timer--;
...
void Fairy::render(gfw::BaseMedia& media, jnr::JumpnRunData& data)
{
	BaseGraphics* graphics = media.getGraphics();
	int offset_x = age % 8 < 4;
	int dy = age % 30 < 15;
