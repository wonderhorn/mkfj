...
RegisterMoverClass(Candle);

void Candle::initialize(int refx, int refy
	, gmtr::Vector2D p, OWNERHANDLE owner, bool turned)
{
	...
	attack_direction = -1;
	this->turned = turned;
	this->speed_walk = this->speed_dash = 0.5;

	writeName("character_enemy_candle", name);
}

void Candle::drive(phsc::Physics& phys, jnr::JumpnRunData& data)
{
...
void Candle::actz(phsc::Physics& phys, jnr::JumpnRunData& data)
{
	/*auto v = $V::polar(2.0, M_PI / 4.0 * attack_direction);
	...
	Fire* b = gfw::generate<Fire>(*tl);
	b->initialize(REFPOINT_FIRE_X, REFPOINT_FIRE_Y, Center() + v + $V(-16, -16.0), Owner(), 12, v, 0);
	$P.sound_stack.push_back($P.snd_ignition);*/

	//phys.v.y = -speed_jump;
	status = eStatus::Casting;
	animation.reset(refx , refy, 80, 1, width, height);
	animation.setSpell(spl::eSpell::spl_fire, 60);
	//animation.setSpell(spl::eSpell::spl_bullet, 15);
	timer_stun.reset(120);
	v_target = $V(0, 0);
}

void Candle::run(jnr::JumpnRunData& data)
{
...
void Candle::think(jnr::JumpnRunData& data)
{
	if (ai_timer)
	{
		ai_timer--;
...
void Candle::render(gfw::BaseMedia& media, jnr::JumpnRunData& data)
{
