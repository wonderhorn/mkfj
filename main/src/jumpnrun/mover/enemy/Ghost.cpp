...
void Ghost::initialize(int refx, int refy
	, gmtr::Vector2D p, OWNERHANDLE owner, bool turned)
{
	...
	writeName("character_enemy_ghost", name);
}

void Ghost::drive(phsc::Physics& phys, jnr::JumpnRunData& data)
{
...
void Ghost::run(jnr::JumpnRunData& data)
{
	Enemy::run(data);
...
void Ghost::think(jnr::JumpnRunData& data)
{
	if (ai_timer)
	{
		ai_timer--;
...
void Ghost::render(gfw::BaseMedia& media, jnr::JumpnRunData& data)
{
