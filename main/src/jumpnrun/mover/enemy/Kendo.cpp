...
void Kendo::initialize(int refx, int refy
	, gmtr::Vector2D p, OWNERHANDLE owner, bool turned)
{
	...
	writeName("character_enemy_kendo", name);
}

void Kendo::run(jnr::JumpnRunData& data)
{
	Enemy::run(data);
...
void Kendo::actz(phsc::Physics& phys, jnr::JumpnRunData& data)
{
	if (isOnSomething)
	{
		//phys.v.y = -speed_jump;
		status = eStatus::Attacking;
		animation.reset(refx + width * 4, refy, 10, 1, width, height, 30, 1);
...
void Kendo::think(jnr::JumpnRunData& data)
{
	if (v_reaction.x != 0)
	{
