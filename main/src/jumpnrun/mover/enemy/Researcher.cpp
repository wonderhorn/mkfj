...
void Researcher::initialize(int refx, int refy
	, gmtr::Vector2D p, OWNERHANDLE owner, bool turned)
{
	...
	writeName("character_enemy_researcher", name);
}

void Researcher::run(jnr::JumpnRunData& data)
{
	Enemy::run(data);
...
void Researcher::actz(phsc::Physics& phys, jnr::JumpnRunData& data)
{
	if (isOnSomething)
	{
		//phys.v.y = -speed_jump;
		status = eStatus::Attacking;
		animation.reset(refx + width * 4, refy, 40, 2, width, height, 0, 1);
		animation.setSpell(spl::eSpell::spl_gasbomb, 40);
...
void Researcher::think(jnr::JumpnRunData& data)
{
	if (v_reaction.x != 0)
	{
