...
RegisterMoverClass(Fish);

void Fish::initialize(int refx, int refy
	, gmtr::Vector2D p, OWNERHANDLE owner, bool turned)
{
	...
	direction = 3;
	this->turned = turned;
	writeName("character_enemy_fish", name);
}

void Fish::drive(phsc::Physics& phys, jnr::JumpnRunData& data)
{
...
void Fish::run(jnr::JumpnRunData& data)
{
	phys.mass = this->location == 'w' ? 0 : 1;
	speed_walk = speed_dash = this->location == 'w' ? 1 : 0.5;
	Enemy::run(data);
	status = eStatus::Flying;

}

void Fish::think(jnr::JumpnRunData& data)
{
	if (ai_timer)
	{
		ai_timer--;
...
void Fish::render(gfw::BaseMedia& media, jnr::JumpnRunData& data)
{
	BaseGraphics* graphics = media.getGraphics();
	int offset_x = age % 90 < 45;
	//int dy = age % 30 < 15;
